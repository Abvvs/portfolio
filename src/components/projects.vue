...
<template>
  <section class="bg-gray-800 pb-4" id="projects">
    ...
    >
      <div>
        <h2 class="title text-indigo-500" data-aos="fade-down">Projects</h2>
        <h4 class="subtitle" data-aos="fade-down">MY CREATION</h4>
        <br />
      </div>
      <div class="flex items-center lg:flex-row flex-col-reverse gap-5">
        <img
          ...
          alt="..."
          data-aos="fade-right"
          class="max-w-[45vw] min-w-[22rem] bg-gray-950 rounded-3xl text-indigo-800"
        />
        <Swiper
          :pagination="{ clickable: true }"
          :space-between="20"
          :modules="[Pagination]"
          class="rounded-3xl pb-16 max-w-xs shadow-lg shadow-indigo-700 self-start"
        >
          <SwiperSlide
            v-for="(item, index) in projectsItems"
            :key="index"
            class="bg-gray-950 rounded-3xl p-5 border-b-8 border-indigo-800 h-fit"
          >
